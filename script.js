...
const ThemeManager = {
    ...
    updateIcons(theme) {
        ...
        if (this.elements.iconMobile) this.elements.iconMobile.className = iconClass;
        ...
        if (this.elements.toggleMobile) {
            ...
            this.elements.toggleMobile.setAttribute('aria-pressed', theme === 'dark' ? 'true' : 'false');
        }
    },
...
const ContactFormManager = {
    ...
    handleResponse(result) {
        if (result.status === 'ok') {
            ...
            const type = result.message?.includes('registro pendiente') ? 'info' : 'error';
    ...
    showMessage(message, type) {
        ...
        messageDiv.style.cssText = `
            ...
            font-weight: 500;
            text-align: center;
            background: ${this.config.messageTypes[type] || this.config.messageTypes.info};
            color: white;
            opacity: 0;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    mobileMenuBtn.addEventListener('click', function() {
        ...
        this.setAttribute('aria-label', !isOpen ? 'Cerrar menú de navegación' : 'Abrir menú de navegación');
